...
test_data2 = """.........................3.......................................94...............806....................596.........793...........186......
...
def check_up(row,col):
    if data[row-1][col].isnumeric():
        found_num = data[row-1][col]
        print('fn ',found_num)
        num = re.findall(r"(\d+)", data[row-1][col-3:col+3])
        print(num)
        print("Looking up")
        for n in num:
            if found_num in n:
                print(n)
                # print("symbol coordinates: ",row+1,col)
                # print("up")
...
def check_down(row,col):
    if data[row+1][col].isnumeric():
        found_num = data[row+1][col]
        # print(data[row+1][col-2:col+3])
        num = re.findall(r"(\d+)", data[row+1][col-3:col+3])
        print(f"fn {found_num}")
        print(num)
        for n in num:
            if found_num in n:
                # print("symbol coordinates: ",row+1,col)
...
def check_top_right(row,col):
    if data[row-1][col+1].isnumeric():
        found_num = data[row-1][col+1]
        print(f"fn: {found_num}")
        num = re.findall(r"(\d+)", data[row-1][col:col+5])
        print(num)
        for n in num:
            if found_num in n:
                print("success")
...
def check_top_left(row,col):
    if data[row-1][col-1].isnumeric():
        num = re.findall(r"(\d+)", data[row-1][col-3:col+1])
        found_num = data[row-1][col-1]
        print(f'fn: {found_num}')
        for n in num:
            if found_num in n:
...
def check_right(row,col):
    if data[row][col+1].isnumeric():
        num = re.findall(r"(\d+)", data[row][col: col + 4])
        if num:
            # print("symbol coordinates: ",row+1,col)
            # print("right")
...
for row_num in rows_2_check.keys():
    for col in rows_2_check[row_num]:
        ...
        # print(f"Checking row: {row_num+1} col: {col+1}")
        ...
        if up:
            ...
            # print("checking top left/right")
            top_right = check_top_right(row_num, col)
            top_left = check_top_left(row_num, col)
            if top_right:
                nums.append(top_right)
            elif top_left:
                nums.append(top_left)
        if down:
            nums.append(down)
        else:
            # print("checking bottom left/right")
            down_right = check_bottom_right(row_num, col)
            down_left = check_bottom_left(row_num, col)
            if down_right:
                nums.append(down_right)
            elif down_left:
                nums.append(down_left)
...
print("engine_parts: ",nums)
print(len(nums))
print("sum of eng parts: ",sum(nums))